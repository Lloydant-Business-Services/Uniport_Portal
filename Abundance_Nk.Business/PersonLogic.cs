...
namespace Abundance_Nk.Business
{
    public class PersonLogic : BusinessBaseLogic<Person, PERSON>
    {
        ...
        public async Task<bool> Modify(Person person, long PersonId)
        {
            try
            {
                Expression<Func<PERSON, bool>> selector = p => p.Person_Id == person.Id;
                PERSON personEntity = await GetEntityByAsync(selector);
        ...
        public bool Modify(Person person)
        {
            try
            {
                ...
                if (person.IDCardNumber != null && !string.IsNullOrEmpty(person.IDCardNumber))
                {
                    personEntity.ID_Card_Number = person.IDCardNumber;
                }
        ...
        private bool CreateAudit(Person person, PersonAudit audit, PERSON personEntity)
        {
            try
            {
                ...
                audit.DateEntered = DateTime.Now;
        ...
        public Person GetPersonByMatricNumber(string matricNumber)
        {
            try
            {
                ...
                {
                    StudentLogic studentLogic = new StudentLogic(); List<Student> students = studentLogic.GetModelsBy(s => s.Matric_Number == matricNumber);
