...
namespace Abundance_Nk.Business
{
    public class ApplicationFormSettingLogic : BusinessBaseLogic<ApplicationFormSetting, APPLICATION_FORM_SETTING>
    {
        ...
        public ApplicationFormSetting GetBy(Session session, Payment payment)
        {
            try
            {
                return
                    GetModelsBy(m => m.Session_Id == session.Id && m.Fee_Type_Id == payment.FeeType.Id).FirstOrDefault();
            }
            catch (Exception)
            {
                throw;
