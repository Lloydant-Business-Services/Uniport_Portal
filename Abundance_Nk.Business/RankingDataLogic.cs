...
namespace Abundance_Nk.Business
{
    ...
    {
        private readonly AdmissionCriteriaForOLevelSubjectAlternativeLogic
            admissionCriteriaForOLevelSubjectAlternativeLogic;

        private readonly AdmissionCriteriaForOLevelSubjectLogic admissionCriteriaForOLevelSubjectLogic;
        private readonly AdmissionCriteriaForOLevelTypeLogic admissionCriteriaForOLevelTypeLogic;
        private readonly OLevelResultDetailLogic oLevelResultDetailLogic;
        private readonly OLevelResultLogic oLevelResultLogic;
        ...
        public string RankAllStudents(APPLICANT_APPLIED_COURSE appliedCourse)
        {
            try
            {
                ...
                if (!(admissionCriteria == null || admissionCriteria.Id <= 0))
                {
                    ...
                    Expression<Func<ADMISSION_CRITERIA_FOR_O_LEVEL_TYPE, bool>> otSelector =
                        ac => ac.Admission_Criteria_Id == admissionCriteria.Id;
                    List<ADMISSION_CRITERIA_FOR_O_LEVEL_TYPE> requiredOlevelTypes =
                        admissionCriteriaForOLevelTypeLogic.GetEntitiesBy(otSelector);

                    //get admission criteria olevel subject
                    Expression<Func<ADMISSION_CRITERIA_FOR_O_LEVEL_SUBJECT, bool>> scSelector =
                        s => s.Admission_Criteria_Id == admissionCriteria.Id && s.Is_Compulsory;
                    Expression<Func<ADMISSION_CRITERIA_FOR_O_LEVEL_SUBJECT, bool>> osSelector =
                        s => s.Admission_Criteria_Id == admissionCriteria.Id && s.Is_Compulsory == false;
                    List<ADMISSION_CRITERIA_FOR_O_LEVEL_SUBJECT> compulsorySubjects =
                        admissionCriteriaForOLevelSubjectLogic.GetEntitiesBy(scSelector);
                    List<ADMISSION_CRITERIA_FOR_O_LEVEL_SUBJECT> otherSubjects =
                        admissionCriteriaForOLevelSubjectLogic.GetEntitiesBy(osSelector);


                    //APPLICANT
                    //get applicant result
                    Expression<Func<APPLICANT_O_LEVEL_RESULT, bool>> firstSittingOlevelSelector =
                        f => f.Person_Id == applicant.Person_Id && f.O_Level_Exam_Sitting_Id == 1;
                    Expression<Func<APPLICANT_O_LEVEL_RESULT, bool>> secondSittingOlevelSelector =
                        s => s.Person_Id == applicant.Person_Id && s.O_Level_Exam_Sitting_Id == 2;
                    APPLICANT_O_LEVEL_RESULT firstSittingOlevelResult =
                        oLevelResultLogic.GetEntityBy(firstSittingOlevelSelector);
                    APPLICANT_O_LEVEL_RESULT secondSittingOlevelResult =
                        oLevelResultLogic.GetEntityBy(secondSittingOlevelSelector);
                    ...
                    {
                        Expression<Func<APPLICANT_O_LEVEL_RESULT_DETAIL, bool>> firstSittingSubjectSelector =
                            fs => fs.Applicant_O_Level_Result_Id == firstSittingOlevelResult.Applicant_O_Level_Result_Id;
                    ...
                    {
                        Expression<Func<APPLICANT_O_LEVEL_RESULT_DETAIL, bool>> secondSittingSubjectSelector =
                            ss =>
                                ss.Applicant_O_Level_Result_Id == secondSittingOlevelResult.Applicant_O_Level_Result_Id;
                    ...
                    if ((firstSittingResultDetails == null || firstSittingResultDetails.Count <= 0) &&
                        (secondSittingResultDetails == null || secondSittingResultDetails.Count <= 0))
                    {
                        return "No O-Level result found for applicant!";
                    }

                    string oLevelTypeRejectReason = null;
                        //CheckOlevelType(requiredOlevelTypes, firstSittingOlevelResult, secondSittingOlevelResult, applicantAppliedCourse);
                    string requiredSubjectsRejectReason = CheckRequiredOlevelSubjects(compulsorySubjects, otherSubjects,
                        firstSittingResultDetails, secondSittingResultDetails, applicantAppliedCourse);
        ...
        private List<APPLICANT_O_LEVEL_RESULT_DETAIL> GetConsolidatedApplicantOlevelResultAboveOrEqualToC6(
            List<APPLICANT_O_LEVEL_RESULT_DETAIL> firstSittingResultDetails,
            List<APPLICANT_O_LEVEL_RESULT_DETAIL> secondSittingResultDetails)
        {
            try
            {
                List<APPLICANT_O_LEVEL_RESULT_DETAIL> firstSittingOLevelResultsAboveOrEqualToC6 =
                    GetOLevelResultsAboveOrEqualToC6(firstSittingResultDetails);
                List<APPLICANT_O_LEVEL_RESULT_DETAIL> secondSittingOLevelResultsAboveOrEqualToC6 =
                    GetOLevelResultsAboveOrEqualToC6(secondSittingResultDetails);

                var consolidatedOLevelResultsAboveOrEqualToC6 = new List<APPLICANT_O_LEVEL_RESULT_DETAIL>();
                if (firstSittingOLevelResultsAboveOrEqualToC6 != null &&
                    firstSittingOLevelResultsAboveOrEqualToC6.Count > 0)
                {
                    if (secondSittingOLevelResultsAboveOrEqualToC6 != null &&
                        secondSittingOLevelResultsAboveOrEqualToC6.Count > 0)
                    {
                        foreach (
                            APPLICANT_O_LEVEL_RESULT_DETAIL oLevelResultDetail in
                                firstSittingOLevelResultsAboveOrEqualToC6)
                        {
                            APPLICANT_O_LEVEL_RESULT_DETAIL oLevelResultDetailMatch =
                                secondSittingOLevelResultsAboveOrEqualToC6.Where(
                                    ss => ss.O_Level_Subject_Id == oLevelResultDetail.O_Level_Subject_Id)
                                    .SingleOrDefault();
                        ...
                        foreach (
                            APPLICANT_O_LEVEL_RESULT_DETAIL oLevelResultDetail in
                                secondSittingOLevelResultsAboveOrEqualToC6)
                        {
                            APPLICANT_O_LEVEL_RESULT_DETAIL oLevelResultDetailMatch =
                                firstSittingOLevelResultsAboveOrEqualToC6.Where(
                                    ss => ss.O_Level_Subject_Id == oLevelResultDetail.O_Level_Grade_Id)
                                    .SingleOrDefault();
                            if (oLevelResultDetailMatch == null)
                            {
                                consolidatedOLevelResultsAboveOrEqualToC6.Add(oLevelResultDetail);
        ...
        private List<APPLICANT_O_LEVEL_RESULT_DETAIL> GetOLevelResultsAboveOrEqualToC6(
            List<APPLICANT_O_LEVEL_RESULT_DETAIL> resultDetails)
        {
            try
            {
                var oLevelResultsAboveOrEqualToC6 = new List<APPLICANT_O_LEVEL_RESULT_DETAIL>();
        ...
        private string CheckRequiredOlevelSubjects(List<ADMISSION_CRITERIA_FOR_O_LEVEL_SUBJECT> compulsorySubjects,
            List<ADMISSION_CRITERIA_FOR_O_LEVEL_SUBJECT> otherSubjects,
            List<APPLICANT_O_LEVEL_RESULT_DETAIL> firstSittingResultDetails,
            List<APPLICANT_O_LEVEL_RESULT_DETAIL> secondSittingResultDetails,
            APPLICANT_APPLIED_COURSE applicantAppliedCourse)
        {
            try
            {
                decimal Score = 0;
                decimal Total = 0;
                int k = 0;
                var RankSubjects = new ArrayList();
                var coreSubjects = new string[10];
                var noncoreSubjects = new string[10];

                //check for the compulsory subjects
                if ((compulsorySubjects == null || compulsorySubjects.Count <= 0) &&
                    (otherSubjects == null || otherSubjects.Count <= 0))
                {
                    throw new Exception("Subject has not been set in Admission Criteria for " +
                                        applicantAppliedCourse.PROGRAMME.Programme_Name.ToUpper() + " in " +
                                        applicantAppliedCourse.DEPARTMENT.Department_Name +
                                        "! Please contact you system administrator");
                }


                int minimumNoOfSubjectWithCreditOrAbove =
                    compulsorySubjects[0].ADMISSION_CRITERIA.Minimum_Required_Number_Of_Subject;

                List<APPLICANT_O_LEVEL_RESULT_DETAIL> applicantConsolidatedResultsWithCreditOrAbove =
                    GetConsolidatedApplicantOlevelResultAboveOrEqualToC6(firstSittingResultDetails,
                        secondSittingResultDetails);
                if (applicantConsolidatedResultsWithCreditOrAbove == null ||
                    applicantConsolidatedResultsWithCreditOrAbove.Count <= 0)
                {
                    return "Required minimum number of subjects at Credit level for " +
                           applicantAppliedCourse.DEPARTMENT.Department_Name.ToUpper() + " is " +
                           minimumNoOfSubjectWithCreditOrAbove + " and the applicant got none, ";
                }
                if (applicantConsolidatedResultsWithCreditOrAbove.Count < minimumNoOfSubjectWithCreditOrAbove)
                {
                    return "Required minimum number of subjects at Credit level for " +
                           applicantAppliedCourse.DEPARTMENT.Department_Name.ToUpper() + " is " +
                           minimumNoOfSubjectWithCreditOrAbove + " the applicant got only " +
                           applicantConsolidatedResultsWithCreditOrAbove.Count + ".";
                }
                ...
                var requiredSubjects = new List<string>();
                foreach (ADMISSION_CRITERIA_FOR_O_LEVEL_SUBJECT compulsoryOLevelSubject in compulsorySubjects)
                {
                    ...
                    var oLevelResultDetailAlternatives = new List<APPLICANT_O_LEVEL_RESULT_DETAIL>();
                    APPLICANT_O_LEVEL_RESULT_DETAIL oLevelResultDetail =
                        applicantConsolidatedResultsWithCreditOrAbove.Where(
                            cs => cs.O_Level_Subject_Id == compulsoryOLevelSubject.O_Level_Subject_Id).FirstOrDefault();

                    List<ADMISSION_CRITERIA_FOR_O_LEVEL_SUBJECT_ALTERNATIVE> compulsoryOLevelSubjectAlternatives =
                        admissionCriteriaForOLevelSubjectAlternativeLogic.GetEntitiesBy(
                            m =>
                                m.Admission_Criteria_For_O_Level_Subject_Id ==
                                compulsoryOLevelSubject.O_Level_Subject_Id).ToList();


                    if (compulsoryOLevelSubjectAlternatives != null && compulsoryOLevelSubjectAlternatives.Count > 0)
                    {
                        foreach (
                            ADMISSION_CRITERIA_FOR_O_LEVEL_SUBJECT_ALTERNATIVE compulsoryOLevelSubjectAlternative in
                                compulsoryOLevelSubjectAlternatives)
                        {
                            APPLICANT_O_LEVEL_RESULT_DETAIL oLevelResultDetailAlternative =
                                applicantConsolidatedResultsWithCreditOrAbove.Where(
                                    cs => cs.O_Level_Subject_Id == compulsoryOLevelSubjectAlternative.O_Level_Subject_Id)
                                    .SingleOrDefault();
                    ...
                    if (oLevelResultDetail == null && oLevelResultDetailAlternatives.Count <= 0)
                    {
                        ...
                        {
                            foreach (
                                ADMISSION_CRITERIA_FOR_O_LEVEL_SUBJECT_ALTERNATIVE alternative in
                                    compulsoryOLevelSubjectAlternatives)
                            {
                        ...
                        {
                            requiredSubjects.Add(" " +
                                                 compulsoryOLevelSubject.O_LEVEL_SUBJECT.O_Level_Subject_Name.ToUpper() +
                                                 alternativeSubjects.ToUpper());
                    ...
                    else
                    {
                        ...
                        {
                            coreSubjects[compulsorySubjectMatchCount] =
                                oLevelResultDetail.O_LEVEL_SUBJECT.O_Level_Subject_Name;
                            RankSubjects.Add(oLevelResultDetail.O_LEVEL_SUBJECT.O_Level_Subject_Name + " : " +
                                             oLevelResultDetail.O_LEVEL_GRADE.O_Level_Grade_Name + " * ");
                        ...
                        {
                            var alternativeResultMatch = new List<APPLICANT_O_LEVEL_RESULT_DETAIL>();
                            alternativeResultMatch =
                                oLevelResultDetailAlternatives.OrderBy(o => o.O_Level_Grade_Id).ToList();
                            coreSubjects[compulsorySubjectMatchCount] =
                                alternativeResultMatch[0].O_LEVEL_SUBJECT.O_Level_Subject_Name;
                            RankSubjects.Add(alternativeResultMatch[0].O_LEVEL_SUBJECT.O_Level_Subject_Name + " : " +
                                             alternativeResultMatch[0].O_LEVEL_GRADE.O_Level_Grade_Name + " * ");
                ...
                if (requiredSubjects.Count > 0)
                {
                    ...
                    rejectReason += missingSubjects + " is a compulsory requirement for " +
                                    applicantAppliedCourse.DEPARTMENT.Department_Name.ToUpper() +
                                    ", but was not obtained by the applicant.";
                }
                ...
                foreach (ADMISSION_CRITERIA_FOR_O_LEVEL_SUBJECT otherSubject in otherSubjects)
                {
                    ...
                    List<APPLICANT_O_LEVEL_RESULT_DETAIL>
                        applicantConsolidatedResultsWithCreditOrAboveWithOutCompolsarySubjects =
                            applicantConsolidatedResultsWithCreditOrAbove;

                    foreach (string subject in coreSubjects)
                    {
                        APPLICANT_O_LEVEL_RESULT_DETAIL oLevelResultDetailx =
                            applicantConsolidatedResultsWithCreditOrAboveWithOutCompolsarySubjects.Where(
                                s => s.O_LEVEL_SUBJECT.O_Level_Subject_Name == subject).FirstOrDefault();
                        applicantConsolidatedResultsWithCreditOrAboveWithOutCompolsarySubjects.Remove(
                            oLevelResultDetailx);
                    }

                    applicantConsolidatedResultsWithCreditOrAboveWithOutCompolsarySubjects =
                        applicantConsolidatedResultsWithCreditOrAboveWithOutCompolsarySubjects.OrderBy(
                            a => a.O_Level_Grade_Id).ToList();
                    APPLICANT_O_LEVEL_RESULT_DETAIL oLevelResultDetail =
                        applicantConsolidatedResultsWithCreditOrAboveWithOutCompolsarySubjects.Where(
                            cs => cs.O_Level_Subject_Id == otherSubject.O_Level_Subject_Id).FirstOrDefault();
                    List<ADMISSION_CRITERIA_FOR_O_LEVEL_SUBJECT_ALTERNATIVE> otherSubjectAlternatives =
                        admissionCriteriaForOLevelSubjectAlternativeLogic.GetEntitiesBy(
                            m => m.Admission_Criteria_For_O_Level_Subject_Id == otherSubject.O_Level_Subject_Id)
                            .ToList();
                    if (otherSubjectAlternatives != null && otherSubjectAlternatives.Count > 0)
                    {
                        foreach (
                            ADMISSION_CRITERIA_FOR_O_LEVEL_SUBJECT_ALTERNATIVE otherSubjectAlternative in
                                otherSubjectAlternatives)
                        {
                            APPLICANT_O_LEVEL_RESULT_DETAIL oLevelResultDetailAlternative =
                                applicantConsolidatedResultsWithCreditOrAboveWithOutCompolsarySubjects.Where(
                                    cs => cs.O_Level_Subject_Id == otherSubjectAlternative.O_Level_Subject_Id)
                                    .FirstOrDefault();
                            if (oLevelResultDetailAlternative != null)
                            {
                    ...
                    if (oLevelResultDetail != null || oLevelResultDetailAlternatives.Count > 0)
                    {
                        ...
                        if (oLevelResultDetail != null)
                        {
                            noncoreSubjects[orderSubjectMatchCount] =
                                oLevelResultDetail.O_LEVEL_SUBJECT.O_Level_Subject_Name;
                            RankSubjects.Add(oLevelResultDetail.O_LEVEL_SUBJECT.O_Level_Subject_Name + " : " +
                                             oLevelResultDetail.O_LEVEL_GRADE.O_Level_Grade_Name);

                            int totalSubjectsOutstanding = minimumNoOfSubjectWithCreditOrAbove -
                                                           compulsorySubjects.Count;
                            string subjectsOutstanding = totalSubjectsOutstanding > 1
                                ? totalSubjectsOutstanding + " subjects"
                                : totalSubjectsOutstanding + " subject";
                ...
                if (otherSubjectCount > orderSubjectMatchCount)
                {
                    if (orderSubjectMatchCount > 0)
                    {
                        rejectReason += "Applicant made a total of " + compulsorySubjectMatchCount +
                                        " compulsory subjects, and " + orderSubjectMatchCount +
                                        " other subject, which totals " + totalSubjectsObtained +
                                        " and does not meet the minimum requirement of " +
                                        minimumNoOfSubjectWithCreditOrAbove + " subjects for " +
                                        applicantAppliedCourse.DEPARTMENT.Department_Name.ToUpper() + ".";
                    }
                    else
                    {
                        var anyOfTheseSubjects = new List<string>();
                        ...
                        string subjectsOutstanding = totalSubjectsOutstanding > 1
                            ...
                            : totalSubjectsOutstanding + " subject";
                        rejectReason += "Applicant made a total of " + compulsorySubjectMatchCount +
                                        " compulsory subjects, but did not obtain any other " + subjectsOutstanding +
                                        " from (" + string.Join(", ", anyOne) +
                                        ") at credit level to complement. Which does not meet the minimum requirement of " +
                                        minimumNoOfSubjectWithCreditOrAbove + " subjects for " +
                                        applicantAppliedCourse.DEPARTMENT.Department_Name.ToUpper() + ".";
                    }
                }
                ...
                var Rank = new RankingData();
                var RankLogic = new RankingDataLogic();
        ...
        private string CheckOlevelType(List<AdmissionCriteriaForOLevelType> requiredOlevelTypes,
            OLevelResult firstSittingOlevelResult, OLevelResult secondSittingOlevelResult,
            AppliedCourse applicantAppliedCourse)
        {
            try
            {
                ...
                {
                    throw new Exception("O-Level Type has not been set in Adission Criteria for " +
                                        applicantAppliedCourse.Programme.Name + " in " +
                                        applicantAppliedCourse.Department.Name +
                ...
                if (firstSittingOlevelResult != null && firstSittingOlevelResult.Id > 0)
                {
                    ...
                    AdmissionCriteriaForOLevelType firstSittingAdmissionCriteriaOLevelType =
                        requiredOlevelTypes.Where(ac => ac.OLevelType.Id == firstSittingOlevelResult.Type.Id)
                            .SingleOrDefault();
                    if (firstSittingAdmissionCriteriaOLevelType == null)
                    {
                        if (secondSittingOlevelResult != null && secondSittingOlevelResult.Id > 0)
                        {
                            ...
                            secondSittingAdmissionCriteriaOLevelType =
                                requiredOlevelTypes.Where(ac => ac.OLevelType.Id == secondSittingOlevelResult.Type.Id)
                                    .SingleOrDefault();
                        }
                    }

                    if (firstSittingAdmissionCriteriaOLevelType == null &&
                        secondSittingAdmissionCriteriaOLevelType == null)
                    {
                        ...
                        //{
                        rejectReason = "'" + applicantOlevelTypes +
                                       "' O-Level Type obtained by the applicant does not meet the O-Level Type (" +
                                       validOlevelTypes + ") requirement for " +
                                       applicantAppliedCourse.Department.Name.ToUpper();
                        //}
                        //else
                        //{
