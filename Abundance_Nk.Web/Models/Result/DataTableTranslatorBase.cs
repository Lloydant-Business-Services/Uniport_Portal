...
namespace Abundance_Nk.Web.Models.Result
{
    public abstract class DataTableTranslatorBase<T>
    {
        ...
        protected bool DuplicateStudentIdExist(DataTable dataTable)
        {
            try
            {
                IEnumerable<object> duplicates = dataTable.AsEnumerable()
                    .GroupBy(r => r[0])
                    .Where(gr => gr.Count() > 1)
                    .Select(g => g.Key);
                ...
                {
                    string idString = "";
                    List<object> ids = duplicates.ToList();
                    ...
                    {
                        SetErrorMessage("Duplicate Student ID of " + idString +
                                        " was detected! Student ID and Admission Number must be unique on the list. Please make necessary corrections, and re-upload.");
                        return true;
                    }
                }
