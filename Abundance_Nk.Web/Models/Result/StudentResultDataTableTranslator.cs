...
namespace Abundance_Nk.Web.Models.Result
{
    public class StudentResultDataTableTranslator :DataTableTranslatorBase<StudentResultDetail>
    {
        ...
        private readonly int maximumScoreObtainable;
        private readonly StudentResultType resultType;
        private CourseLogic courseLogic;

        public StudentResultDataTableTranslator(List<StudentLevel> studentLevels,StudentResultType resultType,
            List<Course> courses,int maximumScoreObtainable)
            : base(studentLevels)
        {
        ...
        public override List<StudentResultDetail> Translate(DataTable excelData)
        {
            try
            {
                ...
                if(InvalidTypeOnSheet(excelData))
                {
                    throw new Exception(ValidationMessage);
                }
                if(DuplicateStudentIdExist(excelData))
                {
                    throw new Exception(ValidationMessage);
                }

                var results = new List<StudentResultDetail>();
                for(int column = 3;column < excelData.Columns.Count;column++)
                {
                    Course course =
                        courses.Where(
                            s => s.Code.Trim().ToLower() == excelData.Columns[column].ColumnName.Trim().ToLower())
                            .FirstOrDefault();
                    if(course == null || course.Id <= 0)
                    {
                        throw new Exception("'" + excelData.Columns[column].ColumnName +
                                            "' cannot be found! Please correct the spelling on the file header and re-upload.");
                    }
                    ...
                    for(int row = 0;row < excelData.Rows.Count;row++)
                    {
                        ...
                        {
                            throw new Exception("No " + studentLevels[0].Level.Name + " student with ID of '" + id +
                                                "' found in the system! Please correct the ID on the sheet, and re-upload.");
                        ...
                        var resultDetail = new StudentResultDetail();
                        resultDetail.Student = new Student();
                        resultDetail.Student.Id = excelData.Rows[row][0] == DBNull.Value
                            ? 0
                            : Convert.ToInt64(excelData.Rows[row][0]);
                        resultDetail.Student.MatricNumber = excelData.Rows[row][1] == DBNull.Value
                            ? null
                            : Convert.ToString(excelData.Rows[row][1]);
                        resultDetail.Student.LastName = excelData.Rows[row][2] == DBNull.Value
                            ? null
                            : Convert.ToString(excelData.Rows[row][2]);
                        resultDetail.Score = excelData.Rows[row][column] == DBNull.Value
                            ? 0
                            : Convert.ToDecimal(excelData.Rows[row][column]);
                ...
                if(errorCount > 0)
                {
                    ValidationMessage = "Error detected on the uploaded file is " + errorCount +
        ...
        protected override void Validate(StudentResultDetail model)
        {
            try
            {
                ...
                {
                    throw new Exception(
                        "Required object not set! Please try again. Contact your system administrator after three unsuccessful tries.");
                }
        ...
        private bool InvalidMatricNumber(StudentResultDetail result)
        {
            try
            {
                ...
                {
                    throw new Exception("Student with ID '" + result.Student.Id +
                                        "' does not exist in the system! Please correct and re-upload.");
                }

                if(student.MatricNumber != result.Student.MatricNumber)
                {
                    throw new Exception("Excel sheet Matric No '" + result.Student.MatricNumber + "' for " +
                                        result.Student.LastName.ToUpper() + " does not match what is in the system '" +
                                        student.MatricNumber + "' ! Please correct and re-upload.");
                }
        ...
        private void InvalidAssessmentScore(StudentResultDetail result)
        {
            try
            {
                ...
                if(resultType != null)
                {
                    ...
                    {
                        errorMessage = "Student " + resultType.Name + " score of '" + result.Score + "' for " +
                                       result.Student.LastName.ToUpper() + " in " + result.Course.Name.ToUpper() + " ( " +
                                       result.Course.Code.ToUpper() +
                                       " ) cannot be greater than Maximum Obtainable Assessment Score of '" +
                                       maximumScoreObtainable + "' ! Please correct and re-upload.";
        ...
        protected override bool InvalidTypeOnSheet(DataTable excelData)
        {
            try
            {
                ...
                for(int row = 0;row < excelData.Rows.Count;row++)
                {
                    ...
                    if(isValidId == false)
                    {
                        ValidationMessage += "Data '" + excelData.Rows[row][0] + "' on row " + rowNumber +
                    ...
                    {
                        ValidationMessage += "Data '" + excelData.Rows[row][1] + "' on row '" + rowNumber +
                                             "', column 2 must be a text! -- ";
                        return true;
                    }
                    ...
                    if(isValidName)
                    {
                        ValidationMessage += "Data '" + excelData.Rows[row][2] + "' on row '" + rowNumber +
                                             "', column 3 must be a text! -- ";
                        return true;
                    }
                    ...
                    for(int column = FIRST_COURSE_COLUMN;column < excelData.Columns.Count;column++)
                    {
                        ...
                        if(isValidScore == false)
                        {
                            ...
                            {
                                ValidationMessage += "Data '" + excelData.Rows[row][column] + "' on row " + rowNumber +
                                                     ", column " + column + " must be a number! -- ";
                                return true;
                            }
                        }
                    }
        ...
        private bool InvalidUploadCriteria()
        {
            try
            {
                ...
                {
                    errorMessage =
                        "No student found for the selected Level, Course of Study and Semester! Please change your selection and try again.";
                ...
                {
                    errorMessage =
                        "Course for the selected Level, Course of Study and Semester has not been set! Please contact your system administrator.";
                    SetErrorMessage(errorMessage);

                    return true;
                }
                if(resultType == null || resultType.Id <= 0)
                {
                ...
                if(maximumScoreObtainable <= 0)
                {
                    errorMessage = "Maximum Score Obtainable for " + resultType.Name +
                                   " has not been set! Please contact your system administrator.";
                    SetErrorMessage(errorMessage);
                ...
                if(maximumScoreObtainable <= 0)
                {
                    errorMessage = "Maximum Percent Score Obtainable for " + resultType.Name +
                                   " has not been set! Please contact your system administrator.";
                    SetErrorMessage(errorMessage);
