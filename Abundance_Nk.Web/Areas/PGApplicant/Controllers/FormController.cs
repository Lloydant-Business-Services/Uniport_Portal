...
namespace Abundance_Nk.Web.Areas.PGApplicant.Controllers
{
    ...
    public class FormController : BaseController
    {
        ...
        public ActionResult PostGraduateApplication()
        {
        ...
        [HttpPost]
        public ActionResult PostGraduateApplication(PGViewModel viewModel)
        {
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    if (payment == null || payment.Id <= 0)
                    {
                        ...
                        if (payment != null && payment.Id > 0)
                        {
                            if (!paystackLogic.ValidateAmountPaid(viewModel.ConfirmationOrderNumber, 115000))
                            {
                                if (!paystackLogic.ValidateAmountPaid(viewModel.ConfirmationOrderNumber, 120000))
                                {
                                    if (!paystackLogic.ValidateAmountPaid(viewModel.ConfirmationOrderNumber, 115000))
                                    {
                                        if (!paystackLogic.ValidateAmountPaid(viewModel.ConfirmationOrderNumber, 115000))
                                        {
                                            SetMessage("Amount paid is not correct!",
                        ...
                        else
                        {
                            ...
                            if (viewModel.ConfirmationOrderNumber.StartsWith("AB"))
                            {
                    ...
                    if (jambRecord != null)
                    {
                        ...
                        if (jambRecord.FirstChoiceInstitution == "ABSU")
                        {
                    ...
                    if (studentJambDetail != null)
                    {
                        pGPaymentViewModel.JambRegistrationNumber = studentJambDetail.JambRegistrationNumber;
                        pGPaymentViewModel.ApplicantJambDetail = studentJambDetail;
                    }

                    if (supplementaryCourse != null)
                    {
                        pGPaymentViewModel.SupplementaryCourse = supplementaryCourse;
                    }

                    pGPaymentViewModel.Payment = payment;
                    if (appliedCourse != null && (appliedCourse.Programme.Id == 1 || appliedCourse.Programme.Id == 3 || appliedCourse.Programme.Id == 4 || appliedCourse.Programme.Id == 7 || appliedCourse.Programme.Id == 15 || appliedCourse.Programme.Id == 16))
                    {
                        ...
                        {
                            pGPaymentViewModel.ApplicationFormSetting = appliedCourse.Setting;
                        }
                    }
                    else
                    {
        ...
        private bool InvalidJambRegistrationNumber(PGPaymentViewModel viewModel)
        {
            const int TEN = 10;
            const string SIX = "9";
        ...
        public ActionResult PostGraduateApplicationInvoiceGeneration()
        {
            PGPaymentViewModel pGPaymentViewModel = new PGPaymentViewModel();

            try
            {
                ViewBag.StateId = pGPaymentViewModel.StateSelectList;
                ViewBag.ProgrammeId = pGPaymentViewModel.ProgrammeSelectListItem;
                ViewBag.FormSettings = pGPaymentViewModel.ApplicationProgrammeSettingSelectListItems;
            ...
            TempData["PostJAMBFormPaymentViewModel"] = pGPaymentViewModel;
            return View(pGPaymentViewModel);
        }

        [HttpPost]
        public ActionResult PostGraduateApplicationInvoiceGeneration(PGPaymentViewModel viewModel)
        {
            try
            {
                ...
                if (ModelState.IsValid)
                {
                            ...
                            new TransactionOptions { IsolationLevel = IsolationLevel.Snapshot }))
                    {
                        ...
                        {
                            //if (viewModel.Programme.Id == 1 || viewModel.Programme.Id == 15 || viewModel.Programme.Id == 16)
                            //{
                            ...
                            //        decimal commission = 1250;
                            //        if (payment.FeeType.Id == 1 && (viewModel.Programme.Id == 15 || viewModel.Programme.Id == 16))
                            //        {
                            //            amount = amount + 150;
                            //            commission = 2250;
                            //        }
                            //        else if (payment.FeeType.Id == 1 && (viewModel.Programme.Id != 15 || viewModel.Programme.Id != 16))
                            //        {
                            //            amount = amount + 250;
                            //            commission = 1250;
                            //        }
                            //        else if (payment.FeeType.Id == 6 || payment.FeeType.Id == 18)
                            //        {
                            //            amount += 200;
                            //            commission = 2300;
                            //        }
                            ...
                            //        PaystackRepsonse paystackRepsonse = paystackLogic.MakePayment(payment, PaystackSecrect, PaystackSubAccount, amount, "N/A", "N/A", "N/A", commission);
                            ...
                            if (viewModel.Programme != null)
                            {
                                ...
                                if (amount <= 10000)
                                {
                                    GetAmountCommission(out addon, out commission, viewModel.Programme, payment.FeeType, payment.Session);
                                    if (commission != 0)
                                    {
                                        payment.Person = person;
                                        PaystackLogic paystackLogic = new PaystackLogic();
                                        ...
                                        decimal Total = payment.FeeDetails.Sum(x => x.Fee.Amount) + 150;
                                        decimal SchoolCut = Total - commission;
                    ...
                    return RedirectToAction("Invoice", "Form",
                        new { Area = "PGApplicant", ivn = viewModel.Payment.InvoiceNumber });
                }
            }
            catch (Exception ex)
            {
        ...
        private bool InvalidFormSettingsSelection(PGPaymentViewModel viewModel)
        {
            try
            {
        ...
        private void KeepApplicationFormInvoiceGenerationDropDownState(PGPaymentViewModel viewModel)
        {
            try
            {
        ...
        public ActionResult Invoice(string ivn)
        {
            try
            {
                ...
                else
                {
                    ...
                    invoice.ProgrammeName = appliedCourse.Programme != null ? appliedCourse.Programme.Name : "";
                    invoice.Session = payment.Session != null ? payment.Session.Name : "";
                }
        ...
        public ActionResult PGApplicationForm()
        {
            ...
            try
            {
                ...
                if (pGPaymentViewModel != null)
                {
                    if (pGPaymentViewModel != null && pGPaymentViewModel.Programme != null)
                    {
                        ...
                        if ((viewModel.Programme.Id == 1 && (pGPaymentViewModel.ApplicationFormSetting.Id == 19 || pGPaymentViewModel.ApplicationFormSetting.Id == 7 || pGPaymentViewModel.ApplicationFormSetting.Id == 9 || pGPaymentViewModel.ApplicationFormSetting.Id == 11 || pGPaymentViewModel.ApplicationFormSetting.Id == 13 || pGPaymentViewModel.ApplicationFormSetting.Id == 17)) || viewModel.Programme.Id == 4)
                        {
                            viewModel.PreviousEducation = null;
                            viewModel.ApplicantJambDetail.JambRegistrationNumber = pGPaymentViewModel.JambRegistrationNumber;
                            viewModel.ApplicantJambDetail = pGPaymentViewModel.ApplicantJambDetail;
                        }
                        else if (viewModel.Programme.Id == 1 && (pGPaymentViewModel.ApplicationFormSetting.Id == 20 || pGPaymentViewModel.ApplicationFormSetting.Id == 8 || pGPaymentViewModel.ApplicationFormSetting.Id == 10 || pGPaymentViewModel.ApplicationFormSetting.Id == 12 || pGPaymentViewModel.ApplicationFormSetting.Id == 14 || pGPaymentViewModel.ApplicationFormSetting.Id == 18))
                        {
                            viewModel.ApplicantJambDetail.JambRegistrationNumber = pGPaymentViewModel.JambRegistrationNumber;
                            viewModel.ApplicantJambDetail = pGPaymentViewModel.ApplicantJambDetail;
                        }
                ...
                if ((applicationform != null && applicationform.Id > 0) && viewModel.ApplicationAlreadyExist == false)
                {
                    ...
                    if (viewModel.AppliedCourse.Programme.Id == 2 || viewModel.AppliedCourse.Programme.Id == 5 ||
                        viewModel.AppliedCourse.Programme.Id == 6 || (viewModel.AppliedCourse.Programme.Id == 1 && (viewModel.ApplicationFormSetting.Id == 20 || viewModel.ApplicationFormSetting.Id == 8 || viewModel.ApplicationFormSetting.Id == 10 || viewModel.ApplicationFormSetting.Id == 12 || viewModel.ApplicationFormSetting.Id == 14 || viewModel.ApplicationFormSetting.Id == 18)))
                    {
        ...
        private void SetDateOfBirth(PGViewModel viewModel)
        {
            try
            {
        ...
        [HttpPost]
        public ActionResult PostJambForm(PGViewModel viewModel)
        {
            var pGPaymentViewModel = (PGPaymentViewModel)TempData["PostJAMBFormPaymentViewModel"];
            ...
            try
            {
                ...
                if (ModelState.IsValid)
                {
                    ...
                    if (pGPaymentViewModel.ApplicantJambDetail != null)
                    {
                        if (pGPaymentViewModel.ApplicantJambDetail.Subject1 != null)
                        {
                            viewModel.ApplicantJambDetail.Subject1 =
                                GetSubject(pGPaymentViewModel.ApplicantJambDetail.Subject1);
                        }
                        else
                        {
                            viewModel.ApplicantJambDetail.Subject1 = GetSubject(viewModel.ApplicantJambDetail.Subject1);
                        }
                        if (pGPaymentViewModel.ApplicantJambDetail.Subject2 != null)
                        {
                            viewModel.ApplicantJambDetail.Subject2 =
                                GetSubject(pGPaymentViewModel.ApplicantJambDetail.Subject2);
                        }
                        else
                        {
                            viewModel.ApplicantJambDetail.Subject2 = GetSubject(viewModel.ApplicantJambDetail.Subject2);
                        }
                        if (pGPaymentViewModel.ApplicantJambDetail.Subject3 != null)
                        {
                            viewModel.ApplicantJambDetail.Subject3 =
                                GetSubject(pGPaymentViewModel.ApplicantJambDetail.Subject3);
                        }
                        else
                        {
                            viewModel.ApplicantJambDetail.Subject3 = GetSubject(viewModel.ApplicantJambDetail.Subject3);
                        }
                        if (pGPaymentViewModel.ApplicantJambDetail.Subject4 != null)
                        {
                            viewModel.ApplicantJambDetail.Subject4 =
                                GetSubject(pGPaymentViewModel.ApplicantJambDetail.Subject4);
                        }
                        else
                        {
        ...
        [HttpPost]
        public ActionResult PostJambPreview(PGViewModel viewModel)
        {
            ...
            try
            {
                if (viewModel.ApplicationAlreadyExist == false)
                {
                    using (var transaction = new TransactionScope(TransactionScopeOption.Required, new TransactionOptions { IsolationLevel = IsolationLevel.Snapshot }))
                    {
                        ...
                        if (existingViewModel.Programme.Id == 1 && (existingViewModel.ApplicationFormSetting.Id == 19 || existingViewModel.ApplicationFormSetting.Id == 7 || existingViewModel.ApplicationFormSetting.Id == 9 || existingViewModel.ApplicationFormSetting.Id == 11 || existingViewModel.ApplicationFormSetting.Id == 13 || existingViewModel.ApplicationFormSetting.Id == 17))
                        {
        ...
        public ActionResult PostJAMBSlip(PGViewModel viewModel)
        {
            var existingViewModel = (PGViewModel)TempData["viewModel"];
        ...
        public ActionResult AcknowledgmentSlip(string FormId)
        {
            try
            {
                ...
                if (viewModel.ApplicationForm != null && viewModel.ApplicationForm.Id > 0)
                {
                    ...
                    viewModel.QRVerification = "http://portal.abiastateuniversity.edu.ng//PGApplicant/Form/AcknowledgmentSlip?FormId=" + FormId;
                    TempData.Clear();
                }
                return View(viewModel);
            }
        ...
        //    }
        //}
        public async void SendSmartSms(ApplicationForm applicationForm, Programme programme)
        {
            try
            {
            ...
            try
            {
                if (existingViewModel == null)
                {
                    ...
                    ViewBag.SupplementaryDepartmentId =
                        Utility.PopulateDepartmentSelectListItemBy(pGPaymentViewModel.Programme);
                    ViewBag.DayOfBirthId = new SelectList(new List<Value>(), ID, NAME);
                    ViewBag.MonthOfBirthId = viewModel.MonthOfBirthSelectList;
                    ViewBag.YearOfBirthId = viewModel.YearOfBirthSelectList;

                    if (pGPaymentViewModel.Programme.Id == 2 || pGPaymentViewModel.Programme.Id == 5 ||
                        pGPaymentViewModel.Programme.Id == 6)
                    {
                        ViewBag.PreviousEducationStartDayId = new SelectList(new List<Value>(), ID, NAME);
                    ...
                    else if (pGPaymentViewModel.Programme.Id == 1 &&
                             pGPaymentViewModel.ApplicationFormSetting.Id == 6 || pGPaymentViewModel.ApplicationFormSetting.Id == 8 || pGPaymentViewModel.ApplicationFormSetting.Id == 10 || pGPaymentViewModel.ApplicationFormSetting.Id == 12 || pGPaymentViewModel.ApplicationFormSetting.Id == 14 || pGPaymentViewModel.ApplicationFormSetting.Id == 18 || pGPaymentViewModel.ApplicationFormSetting.Id == 20)
                    {
                        ...
                        if (pGPaymentViewModel.ApplicantJambDetail != null)
                        {
                            ViewBag.JambScoreId = new SelectList(viewModel.JambScoreSelectList, "Value", "Text",
                                pGPaymentViewModel.ApplicantJambDetail.JambScore);
                            if (pGPaymentViewModel.ApplicantJambDetail.InstitutionChoice != null)
                            {
                                ViewBag.InstitutionChoiceId = new SelectList(viewModel.InstitutionChoiceSelectList,
                                    "Value", "Text",
                                    pGPaymentViewModel.ApplicantJambDetail.InstitutionChoice.Id);
                            }
                            else
                            {
                        ...
                        if (pGPaymentViewModel.ApplicantJambDetail != null)
                        {
                            ...
                            if (pGPaymentViewModel.ApplicantJambDetail.Subject1 != null)
                            {
                                ViewBag.Subject1Id = new SelectList(viewModel.OLevelSubjectSelectList, VALUE, TEXT,
                                    pGPaymentViewModel.ApplicantJambDetail.Subject1.Id);
                            }
                            else
                            {
                                ViewBag.Subject1Id = viewModel.OLevelSubjectSelectList;
                            ...
                            if (pGPaymentViewModel.ApplicantJambDetail.Subject2 != null)
                            {
                                ViewBag.Subject2Id = new SelectList(viewModel.OLevelSubjectSelectList, VALUE, TEXT,
                                    pGPaymentViewModel.ApplicantJambDetail.Subject2.Id);
                            }
                            else
                            {
                                ViewBag.Subject2Id = viewModel.OLevelSubjectSelectList;
                            ...
                            if (pGPaymentViewModel.ApplicantJambDetail.Subject3 != null)
                            {
                                ViewBag.Subject3Id = new SelectList(viewModel.OLevelSubjectSelectList, VALUE, TEXT,
                                    pGPaymentViewModel.ApplicantJambDetail.Subject3.Id);
                            }
                            else
                            {
                                ViewBag.Subject3Id = viewModel.OLevelSubjectSelectList;
                            ...
                            if (pGPaymentViewModel.ApplicantJambDetail.Subject4 != null)
                            {
                                ViewBag.Subject4Id = new SelectList(viewModel.OLevelSubjectSelectList, VALUE, TEXT,
                                    pGPaymentViewModel.ApplicantJambDetail.Subject4.Id);
                            }
                            else
                            {
                                ViewBag.Subject4Id = viewModel.OLevelSubjectSelectList;
                        ...
                        pGPaymentViewModel.Programme.Id == 6)
                    {
                        ...
                        ViewBag.ITDurationId = new SelectList(existingViewModel.ITDurationSelectList, VALUE, TEXT,
                            existingViewModel.PreviousEducation.ITDuration.Id);
                             ...
                             pGPaymentViewModel.ApplicationFormSetting.Id == 6 || pGPaymentViewModel.ApplicationFormSetting.Id == 8 || pGPaymentViewModel.ApplicationFormSetting.Id == 10 || pGPaymentViewModel.ApplicationFormSetting.Id == 14)
                    {
                        SetPreviousEducationEndDateDropDowns(existingViewModel);
                        SetPreviousEducationStartDateDropDowns(existingViewModel);
                        ViewBag.ResultGradeId = new SelectList(existingViewModel.ResultGradeSelectList, VALUE, TEXT,
        ...
        {
            PGPaymentViewModel bankItViewModel = new PGPaymentViewModel();
            try
            {
