...
namespace Abundance_Nk.Web.Areas.PGApplicant.Controllers
{
    ...
    public class PostGraduateFormController : BaseController
    {
        ...
        [HttpPost]
        public ActionResult Index(PGViewModel viewModel)
        {
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    var pGPaymentViewModel = new PGPaymentViewModel();
                    pGPaymentViewModel.Payment = payment;
                    if (appliedCourse != null && (appliedCourse.Programme.Id != 1 || appliedCourse.Programme.Id != 3 || appliedCourse.Programme.Id != 4))
                    {
                        pGPaymentViewModel.Programme = appliedCourse.Programme;
                        pGPaymentViewModel.AppliedCourse = appliedCourse;
                        pGPaymentViewModel.Person = payment.Person;
                        pGPaymentViewModel.Programme = appliedCourse.Programme;
                        
                        pGPaymentViewModel.Initialise();
                    }
                    else
                    {
                        SetMessage(
                    ...
                    TempData["PostJAMBFormPaymentViewModel"] = pGPaymentViewModel;
        ...
        {
            var existingViewModel = (PGViewModel)TempData["viewModel"];
            var pGPaymentViewModel = (PGPaymentViewModel)TempData["PostJAMBFormPaymentViewModel"];
            ApplicantRefereeLogic applicantRefereeLogic = new ApplicantRefereeLogic();
            try
            {
                ...
                if (pGPaymentViewModel != null)
                {
                    if (pGPaymentViewModel != null && pGPaymentViewModel.Programme != null)
                    {
                        viewModel.Session = pGPaymentViewModel.CurrentSession;
                        viewModel.Programme = pGPaymentViewModel.Programme;
                        viewModel.ApplicantJambDetail = null;
                        viewModel.Payment = pGPaymentViewModel.Payment;
                        viewModel.AppliedCourse = pGPaymentViewModel.AppliedCourse;
                        ...
                        viewModel.SupplementaryCourse = pGPaymentViewModel.SupplementaryCourse;
                        viewModel.ApplicantReferees = pGPaymentViewModel.ApplicantReferee;
                        viewModel.ApplicationFormSetting = pGPaymentViewModel.ApplicationFormSetting;
                        viewModel.ApplicationProgrammeFee = pGPaymentViewModel.ApplicationProgrammeFee;
                        viewModel.remitaPyament = pGPaymentViewModel.remitaPayment;
                        viewModel.Person.Id = pGPaymentViewModel.Person.Id;
                        viewModel.Person.FirstName = pGPaymentViewModel.Person.FirstName;
                        viewModel.Person.LastName = pGPaymentViewModel.Person.LastName;
                        viewModel.Person.OtherName = pGPaymentViewModel.Person.OtherName;
                        viewModel.Person.FullName = pGPaymentViewModel.Person.FullName;
                        viewModel.Person.Email = pGPaymentViewModel.Person.Email;
                        viewModel.Person.MobilePhone = pGPaymentViewModel.Person.MobilePhone;
                        viewModel.Person.ContactAddress = pGPaymentViewModel.Person.ContactAddress;
                        viewModel.Person.DateEntered = pGPaymentViewModel.Person.DateEntered;
                        viewModel.Person.State = pGPaymentViewModel.Person.State;
                        viewModel.Person.IDCardNumber = pGPaymentViewModel.Person.IDCardNumber;
                ...
                {
                    viewModel.ApplicantReferees = new List<ApplicantReferee>();
                    if (viewModel.ApplicantReferees.Count < 3)
                    {
                        var difference = 3 - viewModel.ApplicantReferees.Count;
        ...
        private void SetLgaIfExist(PGViewModel viewModel)
        {
            try
            {
        ...
        [HttpPost]
        public ActionResult Form(PGViewModel viewModel)
        {
            var pGPaymentViewModel = (PGPaymentViewModel)TempData["PostJAMBFormPaymentViewModel"];
            ...
            try
            {
                ...
                ModelState["AppliedCourse.Option.Id"].Errors.Clear();
                ModelState["ApplicantPreviousEducations[0].ResultGrade.Id"].Errors.Clear();
                ModelState["ApplicantPreviousEducations[0].StartYear.Id"].Errors.Clear();
                ModelState["ApplicantPreviousEducations[0].EndYear.Id"].Errors.Clear();
                ModelState["ApplicantPreviousEducations[0].Qualification.Id"].Errors.Clear();
                ModelState["ApplicantPreviousEducations[1].ResultGrade.Id"].Errors.Clear();
                ModelState["ApplicantPreviousEducations[1].StartYear.Id"].Errors.Clear();
                ModelState["ApplicantPreviousEducations[1].EndYear.Id"].Errors.Clear();
                ModelState["ApplicantPreviousEducations[1].Qualification.Id"].Errors.Clear();

                ModelState["ApplicantPreviousEducations[2].ResultGrade.Id"].Errors.Clear();
                ModelState["ApplicantPreviousEducations[2].StartYear.Id"].Errors.Clear();
                ModelState["ApplicantPreviousEducations[2].EndYear.Id"].Errors.Clear();
                ModelState["ApplicantPreviousEducations[2].Qualification.Id"].Errors.Clear();

                ModelState["ApplicantPreviousEducations[3].ResultGrade.Id"].Errors.Clear();
                ModelState["ApplicantPreviousEducations[3].StartYear.Id"].Errors.Clear();
                ModelState["ApplicantPreviousEducations[3].EndYear.Id"].Errors.Clear();
                ModelState["ApplicantPreviousEducations[3].Qualification.Id"].Errors.Clear();

                ModelState["ApplicantPreviousEducations[4].ResultGrade.Id"].Errors.Clear();
                ModelState["ApplicantPreviousEducations[4].StartYear.Id"].Errors.Clear();
                ModelState["ApplicantPreviousEducations[4].EndYear.Id"].Errors.Clear();
                ModelState["ApplicantPreviousEducations[4].Qualification.Id"].Errors.Clear();
                ...
                if (ModelState.IsValid)
                {
                    ...
                    {
                        SetPostJAMBStateVariables(viewModel, pGPaymentViewModel);
                        return View(viewModel);
                    }
                    ...
                    // Validate the list of previous tertiary education
                    if (viewModel.ApplicantPreviousEducations?.Count > 0)
                    {
                        foreach(var item in viewModel.ApplicantPreviousEducations)
                        {
                            if (item.SchoolName != null)
                            {
                                viewModel.PreviousEducation = item;
        ...
        private bool InvalidPreviousEducationStartAndEndDate(PreviousEducation previousEducation)
        {
            ...
            try
            {
                ...
                var previousEducationStartDate = new DateTime(previousEducation.StartYear.Id,1, 1);
                ...
                var previousEducationEndDate = new DateTime(previousEducation.EndYear.Id, 1,1);
        ...
        private void SetStudentUploadedPassport(PGViewModel viewModel)
        {
        ...
        private bool InvalidDateOfBirth(PGViewModel viewModel)
        {
            try
            {
        ...
        private void SetPostJAMBStateVariables(PGViewModel viewModel,
            PGPaymentViewModel pGPaymentViewModel)
        {
            try
            {
                TempData["viewModel"] = viewModel;
        ...
        {
            var viewModel = (PGViewModel)TempData["viewModel"];
            ...
            try
            {
                if (viewModel != null)
                {
                    ...
                    for (int i =0; i< viewModel.ApplicantPreviousEducations.Count; i++)
                    {
                        if (viewModel.ApplicantPreviousEducations[i].SchoolName != null)
                        {
                            viewModel.ApplicantPreviousEducations[i].StartDate = new DateTime(viewModel.ApplicantPreviousEducations[i].StartYear.Id, 1, 1);
                            viewModel.ApplicantPreviousEducations[i].EndDate = new DateTime(viewModel.ApplicantPreviousEducations[i].EndYear.Id, 1, 1);
                            viewModel.ApplicantPreviousEducations[i].Qualification = viewModel.EducationalQualifications.Where(m => m.Id == viewModel.ApplicantPreviousEducations[i].Qualification.Id).SingleOrDefault();
                            viewModel.ApplicantPreviousEducations[i].ResultGrade = viewModel.ResultGrades.Where(m => m.Id == viewModel.ApplicantPreviousEducations[i].ResultGrade.Id).SingleOrDefault();
                            viewModel.ApplicantPreviousEducations[i].ITDuration = viewModel.ITDurations.Where(m => m.Id == 2).SingleOrDefault();
                        }
                    }
                    viewModel.ApplicantPreviousEducations.RemoveAll(item => item.SchoolName == null);
                    viewModel.ApplicantReferees.RemoveAll(item => item.Name == null || item.Name == string.Empty);
                    if (viewModel.AppliedCourse?.Option?.Id > 0)
                    {
                        DepartmentOptionLogic departmentOptionLogic = new DepartmentOptionLogic();
                        viewModel.AppliedCourse.Option= departmentOptionLogic.GetModelBy(f => f.Department_Option_Id == viewModel.AppliedCourse.Option.Id);
                    }
                    ...
                    //viewModel.PreviousEducation.MatricNumber = viewModel.PreviousEducation.MatricNumber;
        ...
        [HttpPost]
        public ActionResult FormPreview(PGViewModel viewModel)
        {
            ...
            try
            {
                if (viewModel.ApplicationAlreadyExist == false)
                {
                        ...
                        var transaction = new TransactionScope(TransactionScopeOption.Required, new TransactionOptions { IsolationLevel = IsolationLevel.Snapshot }))
                    {
                        ...
                        //viewModel.Person.IDCardNumber = existingViewModel.Person.IDCardNumber;
                        ...
                        {
                            if (applicantReferee?.Name != string.Empty)
                            {
                        ...
                        if (existingViewModel.ApplicantPreviousEducations?.Count > 0)
                        {
                            for(int i=0; i< existingViewModel.ApplicantPreviousEducations.Count; i++)
                            {
                                if (existingViewModel.ApplicantPreviousEducations[i].SchoolName != null)
                                {
                                    var previousEducationLogic = new PreviousEducationLogic();
                                    existingViewModel.ApplicantPreviousEducations[i].ApplicationForm = newApplicationForm;
                                    existingViewModel.ApplicantPreviousEducations[i].Person = viewModel.Person;
                                    previousEducationLogic.Create(existingViewModel.ApplicantPreviousEducations[i]);
                                }
                            }
                        }
                        

                        //create personmerger if matric number is provided
                        //if (!String.IsNullOrEmpty(existingViewModel.PreviousEducation.MatricNumber))
                        //{
                        //    CreatePersonMerger(existingViewModel.PreviousEducation.MatricNumber, viewModel.Person);
                        //}
                        ...
                        if (personModified)
                        {
                            ...
                            var refereeList = applicantRefereeLogic.GetModelsBy(f => f.Application_Form_Id == newApplicationForm.Id);
                            transaction.Complete();
                            //Send mail to referees
                           
                            if (CheckForInternetConnection())
                            {
                                SendMail(refereeList);
                            }
                ...
                else
                {
                    ...
                    using (var transaction = new TransactionScope(TransactionScopeOption.Required, new TransactionOptions { IsolationLevel = IsolationLevel.Snapshot }))
                    {
                        ...
                        if (existingViewModel.ApplicantPreviousEducations?.Count > 0)
                        {
                            for (int i = 0; i < existingViewModel.ApplicantPreviousEducations.Count; i++)
                            {
                                ...
                                {
                                    var previousEducationLogic = new PreviousEducationLogic();
                                    var lowerCaseSchoolName=existingViewModel.ApplicantPreviousEducations[i].SchoolName.ToLower().Trim();
                                    PreviousEducation previousEducation = previousEducationLogic.GetModelBy(p => p.Application_Form_Id == existingViewModel.ApplicationForm.Id && p.Previous_School_Name.ToLower().Trim()== lowerCaseSchoolName);
                                    if(previousEducation?.Id>0)
                                    {
                                        existingViewModel.ApplicantPreviousEducations[i].Id=previousEducation.Id;
                                        existingViewModel.ApplicantPreviousEducations[i].ApplicationForm = existingViewModel.ApplicationForm;
                                        existingViewModel.ApplicantPreviousEducations[i].Person = viewModel.Person;
                                        previousEducationLogic.Modify(existingViewModel.ApplicantPreviousEducations[i]);
                                    }
                                    else
                                    {
                                        existingViewModel.ApplicantPreviousEducations[i].ApplicationForm = existingViewModel.ApplicationForm;
                                        existingViewModel.ApplicantPreviousEducations[i].Person = existingViewModel.Person;
                                        previousEducationLogic.Create(existingViewModel.ApplicantPreviousEducations[i]);
                                    }
                    ...
                    existingViewModel.ApplicationFormNumber = viewModel.ApplicationForm.Number;
                    TempData["viewModel"] = existingViewModel;
                    return RedirectToAction("AcknowledgmentSlip", "PostGraduateForm", new { Area = "PGApplicant", FormId = Utility.Encrypt(existingViewModel.ApplicationForm.Id.ToString()) });
        ...
        private void SetPersonPassportDestination(PGViewModel existingViewModel, out string junkFilePath,
        ...
        private void SetPersonCredentialDestination(PGViewModel existingViewModel, out string junkFilePath,
            out string destinationFilePath)
        {
            const string TILDA = "~";
        ...
        private Person CreatePerson(PGPaymentViewModel existingViewModel)
        {
            try
            {
        ...
        private Payment CreatePayment(PGPaymentViewModel existingViewModel)
        {
            try
            {
                var payment = new Payment();
        ...
        private AppliedCourse CreateAppliedCourse(PGPaymentViewModel viewModel)
        {
            try
            {
        ...
        private void UpdateOLevelResultDetail(PGViewModel viewModel)
        {
            try
            {
        ...
        private void PopulateAllDropDowns()
        {
            ...
            try
            {
                if (existingViewModel == null)
                {
                    ...
                    ViewBag.SupplementaryDepartmentId = Utility.PopulateDepartmentSelectListItemBy(pGPaymentViewModel.Programme);
                    List<SelectListItem> departmentOptionSelectListItems = Utility.PopulateDepartmentOptionSelectListItem(pGPaymentViewModel.AppliedCourse.Department, pGPaymentViewModel.Programme);
                ...
                else
                {
                    ...
                    if(existingViewModel?.SupplementaryCourse?.Department?.Id>0)
                    ...
                    if(existingViewModel?.DepartmentOptionSelectList==null)
                    existingViewModel.DepartmentOptionSelectList = Utility.PopulateDepartmentOptionSelectListItem(existingViewModel.AppliedCourse.Department, existingViewModel.AppliedCourse.Programme);
        ...
        private void SetDateOfBirthDropDown(PGViewModel existingViewModel)
        {
            try
            {
        ...
        private void SetPreviousEducationStartDateDropDowns(PGViewModel existingViewModel)
        {
            try
            {
        ...
        private void SetPreviousEducationEndDateDropDowns(PGViewModel existingViewModel)
        {
            try
            {
        ...
        private void SetDefaultSelectedSittingSubjectAndGrade(PGViewModel viewModel)
        {
            try
            {
                if (viewModel != null && viewModel.FirstSittingOLevelResultDetails != null)
        ...
        private void SetSelectedSittingSubjectAndGrade(PGViewModel existingViewModel)
        {
            try
            {
        ...
        public void CreatePersonMerger(string MatricNo, Person currentPerson)
        {
            try
            {
                StudentLogic studentLogic = new StudentLogic();
                PersonMergerLogic personMergerLogic = new PersonMergerLogic();
                var previousPerson = studentLogic.GetModelsBy(f => f.Matric_Number == MatricNo).FirstOrDefault();
                if (!String.IsNullOrEmpty(MatricNo) && currentPerson != null && currentPerson.Id > 0 && previousPerson != null && previousPerson.Id > 0)
                {
                    var currentRecord = personMergerLogic.GetModelsBy(g => g.New_Person_Id == currentPerson.Id).FirstOrDefault();
                    using (var transaction = new TransactionScope())
                    {
                        if (currentRecord == null)
                        {
                            PersonMerger personMerger = new PersonMerger()
                            {
                                ...
                                OldPerson = previousPerson

                            };
                            personMergerLogic.Create(personMerger);
                        }
        ...
        public ActionResult RefereeForm(string refereeId)
        {
            PGRefereeViewModel pGRefereeViewModel = new PGRefereeViewModel();
            try
            {
                if(!String.IsNullOrEmpty(refereeId))
                {
                    var rId = Convert.ToInt64(Utility.Decrypt(refereeId));
                    ApplicantRefereeResponseLogic applicantRefereeResponseLogic = new ApplicantRefereeResponseLogic();
                    var existingResponse = applicantRefereeResponseLogic.GetModelsBy(j => j.Applicant_Referee_Id == rId).FirstOrDefault();
                    if (existingResponse?.RefereeResponseId > 0)
                    {
                        SetMessage("You have already responded to this request.", Message.Category.Information);
                        pGRefereeViewModel.Completed = true;
                        return View(pGRefereeViewModel);
                    }
                    ApplicantRefereeLogic applicantRefereeLogic = new ApplicantRefereeLogic();
                    AppliedCourseLogic appliedCourseLogic = new AppliedCourseLogic();
                    RefereeGradingCategoryLogic refereeGradingCategoryLogic = new RefereeGradingCategoryLogic();
                    RefereeGradingSystemLogic refereeGradingSystemLogic = new RefereeGradingSystemLogic();
                    pGRefereeViewModel.ApplicantReferee= applicantRefereeLogic.GetModelsBy(f => f.Applicant_Referee_Id == rId).FirstOrDefault();
                    if (pGRefereeViewModel.ApplicantReferee?.Id > 0)
                    {
                        pGRefereeViewModel.AppliedCourse=appliedCourseLogic.GetModelsBy(f => f.Application_Form_Id == pGRefereeViewModel.ApplicantReferee.ApplicationForm.Id).FirstOrDefault();
                        var gradingCategory=refereeGradingCategoryLogic.GetModelsBy(g => g.Active).ToList();
                        var allgradingSystem=refereeGradingSystemLogic.GetModelsBy(f => f.Active).OrderBy(g => g.Id).ToList();
                        var gradingSystem = allgradingSystem.Where(g => g.Type == 1).ToList();
                        var overallGradingSystem = allgradingSystem.Where(g => g.Type == 2).ToList();
                        pGRefereeViewModel.SetRefereeGradingResponse(gradingCategory, gradingSystem);
                        pGRefereeViewModel.SetOverallGradingSysytem(overallGradingSystem);
        ...
        [HttpPost]
        public ActionResult RefereeForm(PGRefereeViewModel pGRefereeViewModel)
        {
            try
            {
                ApplicantRefereeResponseLogic applicantRefereeResponseLogic = new ApplicantRefereeResponseLogic();
                ApplicantRefereeGradingResponseLogic applicantRefereeGradingResponseLogic = new ApplicantRefereeGradingResponseLogic();
                if (pGRefereeViewModel != null && pGRefereeViewModel.ApplicantReferee?.Id > 0)
                {
                    var existingResponse=applicantRefereeResponseLogic.GetModelsBy(j => j.Applicant_Referee_Id == pGRefereeViewModel.ApplicantReferee.Id).FirstOrDefault();
                    if (existingResponse == null)
                    {
                        using (
                        var transaction = new TransactionScope(TransactionScopeOption.Required, new TransactionOptions { IsolationLevel = IsolationLevel.Snapshot }))
                        {
                            //ApplicantRefereeGradingResponse applicantRefereeGradingResponse = new ApplicantRefereeGradingResponse();
                            ApplicantRefereeResponse applicantRefereeResponse = new ApplicantRefereeResponse();
                            applicantRefereeResponse.ApplicantReferee = new ApplicantReferee() { Id = pGRefereeViewModel.ApplicantReferee.Id };
                            applicantRefereeResponse.CanAcceptApplicant = pGRefereeViewModel.ApplicantRefereeResponse.CanAcceptApplicant;
                            applicantRefereeResponse.DateOfResponse = DateTime.Now;
                            applicantRefereeResponse.DiscloseResponse = pGRefereeViewModel.ApplicantRefereeResponse.DiscloseResponse;
                            applicantRefereeResponse.DurationKnownApplicant = pGRefereeViewModel.ApplicantRefereeResponse.DurationKnownApplicant;
                            applicantRefereeResponse.FullName = pGRefereeViewModel.ApplicantRefereeResponse.FullName;
                            applicantRefereeResponse.RelevantInformation = pGRefereeViewModel.ApplicantRefereeResponse.RelevantInformation;
                            applicantRefereeResponse.Remark = pGRefereeViewModel.ApplicantRefereeResponse.Remark;
                            applicantRefereeResponse.SignatureUrl = pGRefereeViewModel.ApplicantRefereeResponse.SignatureUrl;
                            applicantRefereeResponse.RefereeGradingSystem = new RefereeGradingSystem() { Id= pGRefereeViewModel.OverallRating } ;
                            var creatredResponse =applicantRefereeResponseLogic.Create(applicantRefereeResponse);
                            if (pGRefereeViewModel.RefereeEvaluations?.Count > 0 && creatredResponse?.RefereeResponseId>0)
                            {
                               for(int i=0; i<pGRefereeViewModel.RefereeEvaluations.Count; i++)
                                {
                                    ApplicantRefereeGradingResponse applicantRefereeGradingResponse = new ApplicantRefereeGradingResponse();
                                    applicantRefereeGradingResponse.ApplicantRefereeResponse = creatredResponse;
                                    applicantRefereeGradingResponse.RefereeGradingCategory = new RefereeGradingCategory() { Id = pGRefereeViewModel.RefereeEvaluations[i].GradeCategoryId };
                                    applicantRefereeGradingResponse.RefereeGradingSystem = new RefereeGradingSystem() { Id = pGRefereeViewModel.RefereeEvaluations[i].GradeSystemId };
                                    applicantRefereeGradingResponseLogic.Create(applicantRefereeGradingResponse);
                                }


                            }
                            transaction.Complete();
                            SetMessage("You have successfully responded to Referee Request.", Message.Category.Information);
                            pGRefereeViewModel.Completed = true;
                        }
                    }
                    else
        ...
        public void SendMail(List<ApplicantReferee> applicantReferees)
        {
            try
            {
                ...
                if (applicantReferees?.Count > 0)
                {
                    ...
                    {
                        receipt.RefereeNo = Utility.Encrypt(item.Id.ToString());
                        var templatepath = Server.MapPath("/Areas/Common/Views/Credential/RefereeEmailTemplate.cshtml");
                        receipt.Name = item.ApplicationForm.Person.FullName;
                        EmailMessage message = new EmailMessage();
                        message.Name = item.ApplicationForm.Person.FullName;
        ...
        [HttpPost]
        [AllowAnonymous]
        public JsonResult SetDepartmentList(int programmeId)
        {
            try
            {
                var viewModel = new RefereeFormViewModel();
                viewModel.DepartmentSL = Utility.PopulateDepartmentListItem(programmeId);

                return Json(new { viewModel.DepartmentSL }, JsonRequestBehavior.AllowGet);
            }
            catch(Exception ex) { throw ex; }
        }

        [HttpGet]
        public ActionResult ApplicantRefereeResponse()
        {
            try
            {
                var viewModel = new RefereeFormViewModel();
                ViewBag.SessionSL = Utility.PopulateActiveSessionListItem();
                ViewBag.ProgrammeSL = Utility.PopulatePostGraduateProgrammeSelectListItem(); ;
        ...
        [HttpPost]
        public ActionResult ApplicantRefereeResponse(RefereeFormViewModel viewModel)
        {
            try
            {
                if (viewModel.DepartmentId > 0 && viewModel.SessionId > 0 && viewModel.ProgrammeId > 0)
                {
                    viewModel.Applicants = viewModel.GetApplicantDetails(viewModel.DepartmentId, viewModel.ProgrammeId, viewModel.SessionId);
                }
        ...
        [HttpGet]
        public ActionResult ViewApplicantReferees(long Id)
        {
            try
            {
                if (Id < 1)
                {
                    return RedirectToAction("ApplicantRefereeResponse");
                }

                RefereeFormViewModel viewModel = new RefereeFormViewModel();
                viewModel.ApplicantReferees = viewModel.GetApplicantReferees(Id);
                viewModel.CandidateName = viewModel.GetCandidateNameByFormId(Id);
                viewModel.ApplicationFormId = Id;
        ...
        public ActionResult ViewRefereePrintOut(long Id)
        {
            try
            {
                if (Id < 1)
                ...
                RefereeFormViewModel viewModel = RefereeFormViewModel.SetRefereeDetails(Id);

                if (viewModel == null || string.IsNullOrEmpty(viewModel.DepartmentFacultyName))
                {
                    SetMessage("No Response was Found For This Referee", Message.Category.Information);
                }
                return View(viewModel);
            }
            catch(Exception ex) { throw ex; }
        ...
        public ActionResult ViewMultitplePrintOuts(long Id)
        {
            try
            {
                if (Id < 1)
                ...
                List<RefereeFormViewModel> viewModels = RefereeFormViewModel.SetRefereeDetailsList(Id);

                if (viewModels?.Count < 1)
                {
                    SetMessage("No Responses were Found For This Referee", Message.Category.Information);
                }
