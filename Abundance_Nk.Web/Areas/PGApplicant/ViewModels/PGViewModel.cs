...
namespace Abundance_Nk.Web.Areas.PGApplicant.ViewModels
{
    public class PGViewModel
    {
        ...
        public List<PreviousEducation> ApplicantPreviousEducations { get; set; }
        ...
        private void PopulateAllDropDowns()
        {
            ...
            try
            {
                ...
                Programmes = programmeLogic.GetModelsBy(f=>f.Programme_Name.Contains("PG") && f.Activated==true);
        ...
        public void LoadApplicantionFormBy(Person person, Payment payment)
        {
            try
            {
                ...
                if (ApplicationForm != null && ApplicationForm.Id > 0)
                {
                    ...
                    ApplicantPreviousEducations = previousEducationLogic.GetModelsBy(p => p.Person_Id == person.Id);
                    ...
                    //Ensure the form has space for 3 referees
                        if (ApplicantReferees.Count < 3)
                        {
                            var difference = 3 - ApplicantReferees.Count;
                            for (var i = 0; i < difference; i++)
                            {
                                ApplicantReferees.Add(new ApplicantReferee());
                            }
                        }
                    ...
                    SetAllApplicantPreviousEducation(ApplicantPreviousEducations);
        ...
        public void SetAllApplicantPreviousEducation(List<PreviousEducation> previousEducations)
        {
            try
            {
                ApplicantPreviousEducations = new List<PreviousEducation>();
                if (previousEducations?.Count > 0)
                {
                    for(int i=0; i<5; i++)
                    {
                        PreviousEducation previousEducation = new PreviousEducation();

                        if (i< (previousEducations.Count) && previousEducations[i] != null && previousEducations[i].Id > 0)
                        {
                            
                            previousEducation.SchoolName = previousEducations[i].SchoolName;
                            previousEducation.Course = previousEducations[i].Course;
                            previousEducation.StartDate = previousEducations[i].StartDate;
                            previousEducation.EndDate = previousEducations[i].EndDate;

                            //PreviousEducation.PreND = previousEducation.PreND;
                            //PreviousEducation.PreNDYearFrom = previousEducation.PreNDYearFrom;
                            //PreviousEducation.PreNDYearTo = previousEducation.PreNDYearTo;

                            if (previousEducations[i].ITDuration != null)
                            {
                                previousEducation.ITDuration = previousEducations[i].ITDuration;
                            }
                            else
                            {
                                previousEducation.ITDuration = new ITDuration();
                            ...
                            if (previousEducations[i].ResultGrade != null)
                            {
                                previousEducation.ResultGrade = previousEducations[i].ResultGrade;
                            }
                            else
                            {
                                previousEducation.ResultGrade = new ResultGrade();
                            ...
                            if (previousEducations[i].Qualification != null)
                            {
                                previousEducation.Qualification = previousEducations[i].Qualification;
                            }
                            else
                            {
                                previousEducation.Qualification = new EducationalQualification();
                            ...
                            if (previousEducations[i].StartDate != null)
                            {
                                previousEducation.StartYear = previousEducations[i].StartYear;
                            }
                            else
                            {
                            ...
                            if (previousEducations[i].EndDate != null)
                            {
                                
                                previousEducation.EndYear = previousEducations[i].EndYear;
                            }
                            else
                            {
                        ...
                        ApplicantPreviousEducations.Add(previousEducation);
                    }
                }
