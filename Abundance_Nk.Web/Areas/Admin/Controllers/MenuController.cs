    ...
    public class MenuController :BaseController
    {
        ...
        public ActionResult AddMenu(MenuViewModel viewModel)
        {
            try
            {
                if(viewModel != null)
                {
                    ...
                    List<Menu> MenuList =
                        menuLogic.GetModelsBy(
                            m =>
                                m.Display_Name == viewModel.Menu.Action && m.Controller == viewModel.Menu.Controller &&
                                m.Menu_Group_Id == viewModel.MenuGroup.Id);
                    if(MenuList.Count > 0)
                    {
        ...
        public void RetainDropDown(MenuViewModel viewModel)
        {
            try
            {
                ...
                {
                    ViewBag.MenuGroup = new SelectList(viewModel.MenuGroupSelectList,"Value","Text",
                        viewModel.MenuGroup.Id);
                }
                else
                {
                    ViewBag.MenuGroup = viewModel.MenuGroupSelectList;
            ...
            try
            {
                if(viewModel != null)
                {
                    ...
                    List<MenuInRole> menuInRoleList =
                        menuInRoleLogic.GetModelsBy(
                            m => m.Menu_Id == viewModel.Menu.Id && m.Role_Id == viewModel.Role.Id);
