...
namespace Abundance_Nk.Web.Areas.Admin.Controllers
{
    ...
    public class FeeDetailController : BaseController
    {
        ...
        public ActionResult AllSchoolFees()
        {
            try
            {
                var feeType = new FeeType { Id = 3 };
                var session = new Session { Id = 7 };
                var paymentMode = new PaymentMode { Id = 1 };

                var DepartmentalFees = new List<DepartmentalSchoolFees>();
                var DepartmentalFeeXXX = new List<DepartmentalSchoolFees>();
                var DepartmentalFee = new DepartmentalSchoolFees();

                var levels = new List<Level>();
                var leveLogic = new LevelLogic();
                levels = leveLogic.GetAll();

                var programmeDepartments = new List<ProgrammeDepartment>();
                var programDeptLogic = new ProgrammeDepartmentLogic();
                ...
                foreach (Level level in levels)
                {
                    foreach (ProgrammeDepartment progDept in programmeDepartments)
                    {
                        ...
                        DepartmentalFee.Amount = feeDetailLogic.GetFeeByDepartmentLevel(progDept.Department, level,
                            progDept.Programme, feeType, session, paymentMode);
        ...
        public ActionResult AllFeeDetails()
        {
            try
            {
                var viewModel = new FeeDetailViewModel();
                ViewBag.FeeTypeId = viewModel.FeeTypeSelectListItem;
                    ...
                    case 1:
                        ...
                        for (int i = 0; i < result.FormSettings.Count; i++)
                        {
                            formSettings.Add(new ArrayJsonView()
                            {
                                ...
                                FormSettingName = result.FormSettings[i].Name
                            });
                        }
                        break;
        ...
        public JsonResult CreateRecord(int tableType, string myRecordArray)
        {
            ...
            try
            {
                ...
                AssignJsonArrayValues(arrayJsonView, ref sessionId, ref paymentModeId, ref programmeId, ref departmentId, ref feeTypeId, ref feeId, ref levelId, ref etranzactTypeName, ref terminalId, ref feeName, ref amount, ref feeDescription, ref feeTypeName, ref feeTypeDescription, ref formSettingName);
                ...
                {
                    case 1:
                        result.Message = CreateApplicationFormSetting(sessionId, formSettingName, paymentModeId, userLogic);
                        ...
                        case 1:
                            ...
                            if (id > 0)
                            {
                                ...
                                if (formSetting != null)
                                {
                                    ...
                                    resultModel.Name = formSetting.Name;
                ...
                else if (action == "save")
                {
                    ...
                    switch (tableType)
                    {
                        case 1:
                            ...
                            result.Message = SaveApplicationFormSetting(sessionId, formSettingName, paymentModeId, id);
                            result.IsError = IsError;
                            break;
                        case 2:
        ...
        public void AssignJsonArrayValues(ArrayJsonView arrayJsonView, ref int sessionId, ref int paymentModeId, ref int programmeId, ref int departmentId, ref int feeTypeId, ref int feeId, ref int levelId, ref string etranzactTypeName, ref string terminalId, ref string feeName, ref decimal amount, ref string feeDescription, ref string feeTypeName, ref string feeTypeDescription, ref string formSettingName)
        {
            try
            {
                ...
                if (!string.IsNullOrEmpty(arrayJsonView.FormSettingName))
                {
                    formSettingName = arrayJsonView.FormSettingName;
                }
            }
            catch (Exception)
            {
                ...
                if (feeId > 0 && feeTypeId > 0 && sessionId > 0)
                {
                    ...
                    if (existingFeeDetail == null)
                    {
                        using (TransactionScope scope = new TransactionScope())
                        {
                            ...
                            FeeDetail createdFeeDetail = feeDetailLogic.Create(existingFeeDetail);

                            if (createdFeeDetail != null)
                            {
                                string operation = "INSERT";
                                string action = "ADDED FEE DETAIL";
                                ...
                                feeDetailAudit.DateUploaded = DateTime.Now;
                                feeDetailAudit.User = userLogic.GetModelBy(u => u.User_Name == User.Identity.Name);

                                feeDetailAudit.FeeType = new FeeType() { Id = feeTypeId };
                                feeDetailAudit.Fee = new Fee() { Id = feeId };
                                feeDetailAudit.Session = new Session() { Id = sessionId };

                                if (programmeId > 0 && levelId > 0 && paymentModeId > 0 && departmentId > 0)
                                {
                                    feeDetailAudit.Department = new Department(){ Id = departmentId };
                                    feeDetailAudit.Programme = new Programme(){ Id = programmeId };
                                    feeDetailAudit.Level = new Level(){ Id = levelId };
                                    feeDetailAudit.PaymentMode = new PaymentMode(){ Id = paymentModeId };
                                }
        ...
        public string CreateApplicationFormSetting(int sessionId, string formSettingName, int paymentModeId, UserLogic userLogic)
        {
            string message = "";
            try
            {
                if (sessionId > 0 && paymentModeId > 0)
                {
                    ...
                    ApplicationFormSetting existingFormSetting = applicationFormSettingLogic.GetModelsBy(a => a.Session_Id == sessionId && a.Payment_Mode_Id == paymentModeId && a.Application_Form_Setting_Name == formSettingName).LastOrDefault();
                    ...
                    if (existingFormSetting == null)
                    {
                        ...
                        existingFormSetting.ExamVenue = null;
                        existingFormSetting.PaymentMode = new PaymentMode() { Id = paymentModeId };
                        existingFormSetting.PaymentType = new PaymentType() { Id = 2 };
                        existingFormSetting.PersonType = new PersonType() { Id = 4 };
                        ...
                        existingFormSetting.Name = formSettingName;
        ...
        public string SaveApplicationFormSetting(int sessionId, string formSettingName, int paymentModeId, int id)
        {
            string message = "";
            try
            {
                if (sessionId > 0)
                {
                    ...
                    ApplicationFormSetting existingFormSetting = applicationFormSettingLogic.GetModelsBy(a => a.Session_Id == sessionId && a.Application_Form_Setting_Name.Trim() == formSettingName).LastOrDefault();
                ...
                if (sessionId > 0 && feeId > 0 && feeTypeId > 0)
                {
                    ...
                    if (feeDetail == null)
                    {
                        using (TransactionScope scope = new TransactionScope())
                        {
                            ...
                            string action = "MODIFIED FEE DETAIL";
                            string client = Request.LogonUserIdentity.Name + " (" + HttpContext.Request.UserHostAddress + ")";
                            UserLogic userLogic = new UserLogic();
                            FeeDetailAuditLogic feeDetailAuditLogic = new FeeDetailAuditLogic();
                            ...
                            feeDetailAudit.FeeDetail = feeDetail;
            ...
            try
            {
                ...
                foreach (string file in Request.Files)
                {
                    ...
                    if (CreateFolderIfNeeded(pathForSaving))
                    {
                        ...
                        //var workbook = new XLWorkbook(savedFileName);
                        //var dsFeeDetailList1 = workbook.Worksheet(1);

                        //foreach (IXLRow therow in dsFeeDetailList1.Rows())
                        //{
                        //    if (therow)
                        //    {
                        ...
                        //    {
                        //        string cellValue = thecell.Value.ToString();
                        //    } 
                        ...
                        if (dsFeeDetailList != null && dsFeeDetailList.Tables[0].Rows.Count > 0)
                        {
                            ...
                            for (int i = 0; i < dsFeeDetailList.Tables[0].Rows.Count; i++)
                            {
                                ...
                                if (viewModel.fee.Id > 0 && viewModel.feeType.Id > 0 && viewModel.CurrentSession.Id > 0 && viewModel.Programme != null && viewModel.Programme.Id > 0 && viewModel.Department != null && viewModel.Department.Id > 0 && viewModel.level != null && viewModel.level.Id > 0 && viewModel.paymentMode != null && viewModel.paymentMode.Id > 0)
                                {
                                    ...
                                    {
                                        var feeDetailCheck = feeDetailLogic.GetEntitiesBy(
                                                                                   fd =>
                                                                                       ...
                                                                                       fd.Session_Id == viewModel.CurrentSession.Id).LastOrDefault();
        ...
        public ActionResult SaveUploadedFeeDetail()
        {
            try
            {
                int UploadCount = 0;
                ...
                FeeDetailViewModel viewModel = (FeeDetailViewModel)TempData["viewModel"];
                        ...
                        for (int i = 0; i < viewModel.feeDetails.Count; i++)
                        {
                            using (TransactionScope transactionScope = new TransactionScope())
                            {
                                ...
                                if (createdFeeDetail != null)
                                {
                                    ...
                                    feeDetailAudit.FeeDetail = createdFeeDetail;
                                    feeDetailAudit.PaymentMode = viewModel.feeDetails[i].PaymentMode;
            ...
            SetMessage("No resourse found.", Message.Category.Error);
            return RedirectToAction("UploadFeeDetail");
        }
        ...
        public JsonResult GetDepartments()
        {
            ...
            {
                DepartmentLogic departmentLogic = new DepartmentLogic();
                List<Department> departments = departmentLogic.GetAll().OrderBy(x => x.Name).ToList();
        ...
        public bool CreateFeeDetail(FeeSetup setup, int levelId, int programmeId, List<string> deptIds)
        {
            try
            {
                var logonUsername = User.Identity.Name;
                string operation = "INSERT";
                string action = "CREATED FEE DETAIL";
                string client = Request.LogonUserIdentity.Name + " (" + HttpContext.Request.UserHostAddress + ")";
                FeeDetail feeDetail = new FeeDetail();
                ...
                using (TransactionScope transactionScope = new TransactionScope())
                {
                    foreach (var deptId in deptIds)
                    {
                        ...
                        foreach (var setUpFee in setUpFees)
                        {
                            ...
                            FeeDetail existingFeeDetail = feeDetailLogic.GetModelsBy(f => f.Department_Id == id && f.Fee_Id == feeDetail.Fee.Id && f.Fee_Type_Id == setup.FeeType.Id && f.Level_Id == levelId &&
                                                            f.Programme_Id == programmeId && f.Session_Id == setup.Session.Id && f.Payment_Mode_Id == setup.PaymentMode.Id).LastOrDefault();
                            if (existingFeeDetail == null)
                            {
