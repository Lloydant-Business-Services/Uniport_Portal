...
namespace Abundance_Nk.Web.Areas.Admin.Controllers
{
    ...
    public class RegistrationController :BaseController
    {
        ...
        private readonly StudentUpdateAuditLogic studentUpdateAuditLogic;
        ...
        public ActionResult Index(RegistrationViewModel vModel)
        {
            ...
            if(student != null)
            {
                StudentLevel studentLevel = studentLevelLogic.GetModelBy(p => p.Person_Id == student.Id);

                return RedirectToAction("Form","Registration",
                    new { Area = "Admin",sid = student.Id,pid = studentLevel.Programme.Id });
            }
        ...
        [Authorize(Roles = "admin,Faculty Officer")]
        public ActionResult DisplayAcknowledgmentSlip()
        {
        ...
        [HttpPost]
        public ActionResult DisplayAcknowledgmentSlip(RegistrationViewModel viewModel)
        {
            try
            {
                ...
                Model.Model.Student student = null;
                User user=userLogic.GetModelBy(x => x.User_Name == User.Identity.Name);
                
                if (viewModel.Student.MatricNumber != null)
                {
                    student = studentLogic.GetModelsBy(x => x.Matric_Number == viewModel.Student.MatricNumber).LastOrDefault();
                    if (student != null)
                    {
                        if (user != null && user.Role.Name!="Admin")
                        {
                            var studentDepartment=GetStudentDepartment(student);
                            Staff staff= staffLogic.GetModelBy(x => x.User_Id == user.Id);
                            if (staff == null )
                            {
                                SetMessage("You are not Authorized to view student Detail.",
                            Message.Category.Information);
                                return View(viewModel);
                            }
                            //Ensure that the faculty officer only gain access to students in his faculty
                            if (studentDepartment!=null && staff!=null && staff.Department.Faculty.Id != studentDepartment.Faculty.Id)
                            {
                                SetMessage("The Student Sorted for is not in Your Faculty.",
                            Message.Category.Information);
                                return View(viewModel);
                            }
                        }
                        ...
                        viewModel.StudentSponsor=studentSponsorLogic.GetModelsBy(x => x.Person_Id == student.Id).LastOrDefault();
                        viewModel.NextOfKin = nextOfKinLogic.GetModelBy(x => x.Person_Id == student.Id);
                        viewModel.StudentAcademicInformation=studentAcademicInformationLogic.GetModelsBy(x => x.Person_Id == student.Id).LastOrDefault();
                        viewModel.StudentFinanceInformation= studentFinanceInformationLogic.GetModelsBy(x => x.Person_Id == student.Id).LastOrDefault();
                        viewModel.Person = personLogic.GetModelBy(x => x.Person_Id == student.Id);
                        viewModel.StudentLevel=studentLevelLogic.GetModelsBy(x => x.Person_Id == student.Id).LastOrDefault();
                        var allOlevelResult=oLevelResultLogic.GetModelsBy(x => x.Person_Id == student.Id);
                        viewModel.FirstSittingOLevelResult = new OLevelResult();
                        viewModel.SecondSittingOLevelResult = new OLevelResult();
                        viewModel.FirstSittingOLevelResultDetails = new List<OLevelResultDetail>();
                        viewModel.SecondSittingOLevelResultDetails = new List<OLevelResultDetail>();
                        if (allOlevelResult.Count > 0)
                        {
                            foreach(var olevelResult in allOlevelResult)
                            {
                                if (olevelResult.Sitting.Id == 1)
                                {
                                    var firstSittingDetail=oLevelResultDetailLogic.GetModelsBy(x => x.Applicant_O_Level_Result_Id == olevelResult.Id);
                                    viewModel.FirstSittingOLevelResultDetails.AddRange(firstSittingDetail);
                                    viewModel.FirstSittingOLevelResult = olevelResult;
                                }
                                else
                                {
                                    var secondSittingDetail=oLevelResultDetailLogic.GetModelsBy(x => x.Applicant_O_Level_Result_Id == olevelResult.Id);
                                    viewModel.SecondSittingOLevelResultDetails.AddRange(secondSittingDetail);
                                    viewModel.SecondSittingOLevelResult = olevelResult;
                                }
                        ...
                        if(viewModel.StudentLevel.Programme.Id == 3 || viewModel.StudentLevel.Programme.Id == 4)
                        {
                            viewModel.PreviousEducation=previousEducationLogic.GetModelsBy(x => x.Person_Id == student.Id).LastOrDefault();
                            viewModel.StudentEmploymentInformation=studentEmploymentInformationLogic.GetModelsBy(x => x.Person_Id == student.Id).LastOrDefault();
                            viewModel.StudentNdResult=studentNdResultLogic.GetModelsBy(x => x.Person_Id == student.Id).LastOrDefault();
                        }
        ...
        {
            var studentUpdateAudit = new StudentUpdateAudit();
        ...
        public Department GetStudentDepartment(Model.Model.Student student)
        {
            Department department = new Department();
            try
            {
                StudentLevel studentLevel=studentLevelLogic.GetModelBy(x => x.Person_Id == student.Id);
