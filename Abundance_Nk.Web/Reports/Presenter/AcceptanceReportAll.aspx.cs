...
{
    public partial class AcceptanceReportAll : Page
    {
        ...
        private void DisplayReport(Session session, Department department, Programme programme)
        {
            try
            {
                ...
                List<AcceptanceView> report = paymentLogic.GetAcceptanceReport(session, department, programme, txtBoxDateFrom.Text, txtBoxDateTo.Text);
        ...
        protected void btBulk_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                for (int j = 0; j < programmedepartments.Count; j++)
                {
                    Department currentDepartment = programmedepartments[j];
                    PaymentLogic paymentLogic = new PaymentLogic();
                    var report = paymentLogic.GetAcceptanceReport(SelectedSession, currentDepartment, SelectedProgramme, txtBoxDateFrom.Text, txtBoxDateTo.Text);
                    ...
                    if (report.Count > 0)
                    {
                        ...
                        string savelocation = Path.Combine(path, SelectedProgramme.Name.Replace("/", "_") + " " + currentDepartment.Name.Replace("/", "_") + ".pdf");
