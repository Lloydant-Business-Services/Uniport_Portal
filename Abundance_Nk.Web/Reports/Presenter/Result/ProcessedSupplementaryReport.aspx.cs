...
{
    public partial class ProcessedSupplementaryReport : Page
    {
        ...
        private void DisplayReport(Session session, Department department, InstitutionChoice institutionChoice, ApplicationFormSetting applicationFormSetting)
        {
            try
            {
                ...
                List<Model.Model.ProcessedSupplementaryReport> resultList = resultLogic.GetSupApplicantsByChoice(department, session, institutionChoice, applicationFormSetting);

                string reportPath = "";
                string bind_ds = "DataSet1";
                reportPath = @"Reports\ProcessedSupplementaryReport.rdlc";
                if (applicationFormSetting.Id == 10)
                {
                    reportPath = @"Reports\ProcessedSupplementaryReportDE.rdlc";
                }


                rv.Reset();
                rv.LocalReport.DisplayName = "Processed Supplementary Applicants";
        ...
        protected void Button1_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    var resultLogic = new ApplicantJambDetailLogic();
                    List<Model.Model.ProcessedSupplementaryReport> report = resultLogic.GetSupApplicantsByChoice(department, SelectedSession, SelectedInstitutionChoice, SelectedApplicationFormSetting);
                    ...
                    if (report.Count > 0)
                    {
                        ...
                        string bind_ds = "DataSet1";
                        reportPath = @"Reports\ProcessedSupplementaryReport.rdlc";
                        if (SelectedApplicationFormSetting.Id == 10)
                        {
                            reportPath = @"Reports\ProcessedSupplementaryReportDE.rdlc";
                        }
                        ...
                        rptViewer.Reset();
                        rptViewer.LocalReport.DisplayName = "Processed Supplementary Report";
                ...
                using (var zip = new ZipFile())
                {
                    ...
                    string zipFileName = "SupplementaryApplicantReport";
