...
{
    public partial class PutmeReportOlevelBulk : System.Web.UI.Page
    {
        ...
        private void DisplayReportBy(Department department)
        {
            try
            {
                var putmeResultLogic = new PutmeResultLogic();
                List<PutmeSlip> report = putmeResultLogic.GetPUTMEReportOlevelByChoice(department, 17,new InstitutionChoice(){Id  = SelectedChoice.Id });
                string bind_dsStudentPaymentSummary = "dsReport";
                string reportPath = @"Reports\PUTME_OLEVEL.rdlc";
        ...
        protected void Display_Button_Click1(object sender, EventArgs e)
        {
            try
            {
                ...
                DisplayReportBy(SelectedDepartment);
            }
            catch (Exception ex)
            {
                lblMessage.Text = ex.Message + ex.InnerException.Message;
            ...
            try
            {
                ...
                foreach (Department deptDepartment in departments)
                {
                    foreach (InstitutionChoice institutionChoice in choices)
                    {
                        ...
                        List<PutmeSlip> report = putmeResultLogic.GetPUTMEReportOlevelByChoice(deptDepartment, 11, institutionChoice);
