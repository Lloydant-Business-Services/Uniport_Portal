...
{
    public partial class PutmeReportBulk : System.Web.UI.Page
    {
        ...
        private void DisplayReportBy(Department department,InstitutionChoice choice)
        {
            try
            {
                 ...
                 List<PutmeSlip> report = putmeResultLogic.GetPUTMEReport(department, 17, choice.Id);
                string bind_dsStudentPaymentSummary = "dsReport";
                string reportPath = @"Reports\PUTMEReport.rdlc";
            ...
            try
            {
                ...
                foreach (Department deptDepartment in departments)
                {
                    foreach (InstitutionChoice choice in choices)
                    {
                        ...
                        var putmeResultLogic = new PutmeResultLogic();
                        List<PutmeSlip> report = putmeResultLogic.GetPUTMEReport(deptDepartment, 11, choice.Id);

                        // List<PutmeSlip> report = putmeResultLogic.GetPUTMEReportUnMatched(deptDepartment);
                        if (report.Count > 0)
                        {
